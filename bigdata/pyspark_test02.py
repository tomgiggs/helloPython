...
kafka_strem_context = KafkaUtils.createStream(ssc, "172.17.0.6:2181", "spark_test12", {"flumetest2": 1},
                                              kafkaParams=propertites)
...
kafka_strem_context.map(lambda x: x[1]).map(lambda x: (x.split("|@|")[0], x.split("|@|")[2])).reduceByKey(
    lambda a, b: a + b).foreachRDD(lambda q: q.foreachPartition(toredis))
kafka_strem_context.map(out_data)
kafka_strem_context.map(lambda x: x[1]).count().saveAsTextFiles("/log/test_0",
                                                                '.txt')  # 这个基于流的写出文件的方法是可以运行的，但是如果不是基于流的sparkcontext就不行，原因未知
